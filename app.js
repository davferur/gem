...
async function boot(){
  ...
  if((!had && !SB_OK) || !STATE.creditos || !STATE.creditos.length){
    if(!STATE.creditos || !STATE.creditos.length){
      ...
      // Sembrar pagos del mes en curso para que los indicadores de cumplimiento
      // e informe diario reflejen actividad real desde el arranque.
      if(!STATE.pagos || !STATE.pagos.length){
        STATE.pagos=[];
        const hoyISO=iso(hoy());
        const activos=STATE.creditos.filter(c=>c.estado==="Activo" && c.cuotaValor>0);
        // ~35% de los créditos activos ya pagaron su cuota este mes
        activos.slice(0, Math.max(1, Math.round(activos.length*0.35))).forEach((c,i)=>{
          const ms=metaSplit(c.cuotaValor);
          const dia=Math.min(28, 1+(i%25));
          const fecha=hoyISO.slice(0,8)+String(dia).padStart(2,"0");
          STATE.pagos.push({id:uid(),creditoId:c.id,fecha:fecha+"T10:00:00.000Z",
            capital:ms.capital,interes:ms.interes,mora:0,anticipado:0,
            valor:ms.capital+ms.interes,metodo:(i%2?"Bancolombia":"Efectivo"),
            referencia:"",comprobanteUrl:"",obs:"Pago registrado",responsable:"Sistema"});
        });
      }
      save();
    }
...
function renderDocsPublico(){
  ...
  if(!STATE.documentos) STATE.documentos={};
  g.innerHTML = EMPRESAS_GRUPO.map(e=>{
    const docs=(STATE.documentos[e.n]||[]);
    return `<div class="home-card" style="text-align:left;cursor:default">
      <div style="display:flex;align-items:center;gap:9px;margin-bottom:10px">
        <span style="width:12px;height:12px;border-radius:50%;background:${e.c};display:inline-block;flex-shrink:0"></span>
        <h3 style="margin:0;font-size:15.5px">${e.n}</h3>
      </div>
      ${docs.length
        ? docs.map(d=>`<a href="#" onclick="event.preventDefault();alert('Documento pendiente de cargar por el administrador.')" style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--linea);font-size:13px;color:var(--azul);text-decoration:none">📄 ${d.nombre}</a>`).join("")
        : `<div style="font-size:13px;color:var(--gris);padding:10px 0">📄 Documentación pendiente de publicar.</div>`}
      <div style="margin-top:12px;font-size:11.5px;color:var(--gris-cl,#8A97A5)">Los documentos se publican desde el portal administrativo.</div>
    </div>`;
  }).join("");
}
...
const BLOG_COLORES={"Educativo":["#E8F0F9","#1B6CA8","📘"],"Guía":["#E7F5ED","#1E8E5A","🏠"],"Consejo":["#FDF1E3","#CA6F1E","💡"],"Noticia":["#F3E8F5","#8E44AD","📣"]};
function blogEstilo(cat){ return BLOG_COLORES[cat]||["#F4EFE4","#C6820E","📰"]; }
...
function renderBlogPublico(){
  ...
  g.innerHTML=posts.map((p,i)=>{
    const [bg,fg,ic]=blogEstilo(p.cat);
    return `<a class="blog-card" href="blog-articulo.html?slug=${p.slug||i}">
      <div class="bimg" style="background:${bg}">${ic}</div>
      <div class="bbody">
        <span class="bcat" style="background:${bg};color:${fg}">${p.cat||"Artículo"}</span>
        <h3>${p.titulo}</h3>
        <p>${p.resumen||''}</p>
        <div class="bmeta"><span>${p.fecha?fechaBonita(p.fecha):"GEM"}</span><span class="bmore">Leer más →</span></div>
      </div>
    </a>`;}).join("");
}
...
function renderArticuloDesdeURL(){
  ...
  const posts=blogPosts();
  const idx=posts.findIndex(x=>String(x.slug)===String(slug));
  const p = (idx>=0?posts[idx]:null) || posts[Number(slug)] || posts[0];
  ...
  const cuerpo=(p.cuerpo||p.resumen||"").split("\n").filter(l=>l.trim()).map(l=>{
    const esNum=/^\d+\./.test(l.trim());
    return `<p class="${esNum?'art-num':''}">${l}</p>`;
  }).join("");
  // artículos relacionados (los otros)
  const otros=posts.filter(x=>x!==p).slice(0,2);
  cont.innerHTML=`
    <div class="art-wrap">
      <div class="pg-crumb" style="color:var(--gris)"><a href="index.html" style="color:var(--azul)">Inicio</a> / <a href="blog.html" style="color:var(--azul)">Blog</a> / ${p.titulo}</div>
      <div class="art-cover" style="background:${bg};margin-top:16px">${ic}</div>
      <span class="bcat" style="background:${bg};color:${fg};display:inline-block;font-size:11px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;padding:5px 13px;border-radius:14px;margin-bottom:14px">${p.cat||"Artículo"}</span>
      <h1>${p.titulo}</h1>
      <div class="art-meta"><span>✍️ Por <b>GEM</b></span><span>·</span><span>${p.fecha?fechaBonita(p.fecha):hoy().toLocaleDateString("es-CO")}</span></div>
      <div class="art-body">${cuerpo}</div>
      <div style="margin-top:34px;padding:26px;background:linear-gradient(135deg,var(--petroleo),#154a7a);border-radius:16px;text-align:center;color:#fff">
        <div style="font-weight:800;font-size:17px;margin-bottom:8px">¿Tienes dudas sobre este tema?</div>
        <p style="color:#c5d4e2;font-size:14px;margin-bottom:16px">Un asesor de GEM te orienta sin costo.</p>
        <a href="https://wa.me/573158542836?text=${encodeURIComponent('Hola *GEM* 👋\n\nLeí el artículo *"'+p.titulo+'"* en su blog y quiero recibir *asesoría* sobre este tema.\n\n¿Me pueden ayudar? 🙌')}" target="_blank" style="display:inline-block;background:#25D366;color:#fff;padding:13px 28px;border-radius:24px;font-weight:700;text-decoration:none">💬 Hablar con un asesor</a>
      </div>
      ${otros.length?`<div class="art-rel">
        <h2 style="font-size:20px;color:var(--petroleo);margin-bottom:18px">Sigue leyendo</h2>
        <div class="blog-grid">${otros.map(o=>{const [b2,f2,i2]=blogEstilo(o.cat);
          return `<a class="blog-card" href="blog-articulo.html?slug=${o.slug}">
            <div class="bimg" style="background:${b2};height:110px;font-size:38px">${i2}</div>
            <div class="bbody"><span class="bcat" style="background:${b2};color:${f2}">${o.cat}</span><h3 style="font-size:16px">${o.titulo}</h3></div>
          </a>`;}).join("")}</div>
      </div>`:''}
    </div>`;
}
